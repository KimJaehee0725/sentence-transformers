...
def test_mnrl_bank_updates_and_fifo(stsb_bert_tiny_model):
    model = stsb_bert_tiny_model
    model.to("cpu")
    loss = losses.MultipleNegativesRankingLoss(model, bank_size=2)
    loss.train()
...
def test_mnrl_bank_includes_all_candidates_for_triplets(stsb_bert_tiny_model):
    model = stsb_bert_tiny_model
    model.to("cpu")
    loss = losses.MultipleNegativesRankingLoss(model, bank_size=1)
    loss.train()
...
def test_mnrl_bank_supports_bidirectional(stsb_bert_tiny_model):
    model = stsb_bert_tiny_model
    model.to("cpu")
...
def test_mnrl_bank_rejects_unsupported_direction_sets(stsb_bert_tiny_model):
    model = stsb_bert_tiny_model
...
def test_mnrl_bank_supports_bidirectional_with_gather(stsb_bert_tiny_model):
    model = stsb_bert_tiny_model
    model.to("cpu")
    loss = losses.MultipleNegativesRankingLoss(
        model,
